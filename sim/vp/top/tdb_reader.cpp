//from open-source https://github.com/lizhirui/MyRISC-VCore-rtl/blob/master/tools/tdb_reader/src/tdb_reader.cpp
...
namespace trace
{
    ...
    void trace_reader::open(std::string filename)
    {
        ...
        if(!_is_open)
        {
            std::cout << "Failed to open trace database " << filename << "!" << std::endl;
            assert(false);
        }
        ...
        printf("row num = %llu\n", (long long unsigned int)((file_size - hinfo.data_offset) / hinfo.row_size));
        assert((row_num * hinfo.row_size) == (file_size - hinfo.data_offset));
        db.seekg(hinfo.data_offset, std::ios::beg);
        std::cout << "trace database load ok!" << std::endl;
        std::cout << "---------------------------------" << std::endl;
    }
